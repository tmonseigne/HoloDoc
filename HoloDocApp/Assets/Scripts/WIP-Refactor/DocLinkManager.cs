...
public class DocLinkManager : Singleton<DocLinkManager>
{
	...
	public void OnLinkEnded(GameObject document)
	{
		...
		{
			linkTail = document;
			DocProperties head = linkHead.GetComponent<DocManager>().Properties;
			DocProperties tail = linkTail.GetComponent<DocManager>().Properties;
			if (head.LinkId != -1)
			{
				// The head has a link
				if (tail.LinkId != -1)
				{
					...
					else
					{
						...
						{
							// 1.1: Switch ids
							Links[oldTailLinkId].Objects[i].GetComponent<DocManager>().Properties.LinkId = head.LinkId;
							// 1.2: Switch colors
							Links[oldTailLinkId].Objects[i].GetComponent<DocManager>().SetColor(Links[head.LinkId].LinkColor);
						}
				...
				else
				{
					...
					linkTail.GetComponent<DocManager>().SetColor(Links[head.LinkId].LinkColor);
				}
			}
			else
			{
				...
				if (tail.LinkId != -1)
				{
					...
					linkHead.GetComponent<DocManager>().SetColor(Links[tail.LinkId].LinkColor);
				}
				else
				{
					...
					linkTail.GetComponent<DocManager>().SetColor(Links[linkId].LinkColor);
				}
			}
		}
